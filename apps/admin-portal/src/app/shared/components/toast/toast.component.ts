...
/**
 * Renders the toasts held by {@link ToastService}. Mount once in the app root.
 *
 * Ported from the web app. The animation there was a hand-written `@keyframes`
 * block; here it is Tailwind transition utilities instead, per the project's
    ...
    <div class="fixed top-4 right-4 z-50 flex flex-col gap-3 pointer-events-none max-w-md">
      @for (toast of toastService.toasts$(); track toast.id) {
        <div
          ...
          class="pointer-events-auto shadow-lg rounded-xl p-4 border-2 backdrop-blur-sm transition-all duration-300 ease-out"
          role="alert"
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <button
              ...
              class="flex-shrink-0 rounded-lg p-1.5 inline-flex items-center justify-center transition-colors"
...
export class ToastContainerComponent {
  ...
  protected getToastClasses(type: string): string {
    ...
    return `relative ${typeClasses[type as keyof typeof typeClasses] || typeClasses.info}`;
  }
