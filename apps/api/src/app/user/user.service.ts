...
export class UserService {
  ...
  private async saveAvailabilityOverride(
    ...
    try {
      ...
      const override: AvailabilityOverrideInterface = {
      ...
      const overrides = overrideId
        ? current.map((entry) => (entry.id === overrideId ? override : entry))
        : [...current, override];
  ...
  async deleteAvailabilitySlot(
    ...
    try {
      ...
      if (dto.timeFrameIndex === undefined) {
        ...
        const newFrames = dayEntry.timeFrames.filter(
          (_, i) => i !== dto.timeFrameIndex,
        );
